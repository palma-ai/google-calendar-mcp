...
describe("Google Calendar MCP Tool Calls", () => {
  ...
  it('should handle "update-event" tool call', async () => {
    ...
    (mockCalendarApi.events.get as ReturnType<typeof vi.fn>).mockResolvedValue({
      data: mockEventData,
    });
  ...
  it('should handle "get-freebusy" tool call', async () => {
    ...
    expect(result.content[0].text).toBe(
      "Cannot check availability for test@gmail.com (account not found)"
    );
  });
  ...
  it("should throw ZodError if required args are missing", async () => {
    ...
    await expect(handler.runTool(invalidArgs, {} as any)).rejects.toThrow(
      `Invalid arguments Error: [{"code":"invalid_type","expected":"string","received":"undefined","path":["timeMin"],"message":"Required"},{"code":"invalid_type","expected":"string","received":"undefined","path":["timeMax"],"message":"Required"},{"validation":"email","code":"invalid_string","message":"Must be a valid email address","path":["items",0,"id"]}]`
    );
  });

  it('should handle "create-event" with sendUpdates parameter', async () => {
    // Arrange
    const mockEventArgs = {
      calendarId: "primary",
      summary: "Team Meeting with Notifications",
      description: "Meeting with email notifications",
      start: "2024-08-15T10:00:00-07:00",
      end: "2024-08-15T11:00:00-07:00",
      timeZone: "America/Los_Angeles",
      attendees: [{ email: "test@example.com" }],
  ...
  it('should handle "update-event" with sendUpdates parameter', async () => {
    // Arrange
    const updateEventArgs = {
      calendarId: "primary",
      eventId: "eventToUpdate123",
    ...
    expect(mockCalendarApi.events.patch).toHaveBeenCalledWith({
      ...
      requestBody: {
        summary: updateEventArgs.summary,
        start: { timeZone: updateEventArgs.timeZone },
        end: { timeZone: updateEventArgs.timeZone },
      },
      sendUpdates: "externalOnly",
    });
  ...
  it('should handle "delete-event" with sendUpdates parameter', async () => {
    // Arrange
    const deleteEventArgs = {
      calendarId: "primary",
      eventId: "event123",
